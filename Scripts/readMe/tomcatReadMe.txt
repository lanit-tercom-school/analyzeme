ƒл€ использовани€ .bat файлов локальной сборки, работающих с Tomcat, необходимо:
 ...
 3. «апустить из командной строки (в папке Scripts)
	- дл€ депло€ на http://localhost:8080/analyzeme/ : tomcat.bat
	- дл€ депло€ на на http://localhost:8080/contextPath/ : tomcat_ultimate.bat contextPath
 ...
 –езультат работы доступен по указанному адресу до принудительного андепло€ - средствами томката или удалением из папки %CATALINA_HOME%\webapps вручную
 WARNING: до андепло€ повторный деплой на этот адрес будет недоступен, используйте вариант с передачей другой contextPath в качестве аргумента или сбрасывайте с томката проект