...
public class RFileLinkerTest {
	...
	public void testParse() {
		try {
			...
			ArrayList<String> parsed = RFileLinker.parseForTests(
					"here are some code \n {} : \n some other \n " +
							"x_from_file.json_ \n y_from__repo__MyData.json__ \n " +
							"y <- c(x_from__repo__MyJson.excel__)");
			assertTrue("RFileLinker doesn't work properly",
					result.get(0).equals(parsed.get(0)) &&
							result.get(1).equals(parsed.get(1)));
		} catch (Exception e) {
