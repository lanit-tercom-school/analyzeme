...
public class ScalarFromR implements IFromR<ScalarResult> {
     ...
     */
    public ScalarResult runScript(final String scriptName, final String rScript,
                                  int userId, String projectId) throws Exception {
        if (rScript == null || userId == 0 ||
                projectId == null || projectId.equals("")) {
            throw new IllegalArgumentException();
        }
        return RFacade.runScriptToGetScalar(scriptName, rScript, userId, projectId);
    }
     ...
     */
    public ScalarResult runScript(final String scriptName, final String rScript, final Map<String, List<DataEntry>> data) throws Exception {
        if (rScript == null || rScript.equals("") ||
                data == null) {
            throw new IllegalArgumentException();
        }
        return RFacade.runScriptToGetScalar(scriptName, rScript, data);
    }
}
