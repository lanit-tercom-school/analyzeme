...
public class ScalarFromR implements IFromR<ScalarResult> {
     ...
     */
    public ScalarResult runScript(final Script script,
                                  int userId, String projectId) throws Exception {
        if (script == null || userId == 0 ||
                projectId == null || projectId.equals("")) {
            throw new IllegalArgumentException();
        }
        return RFacade.runScriptToGetScalar(script, userId, projectId);
    }
     ...
     */
    public ScalarResult runScript(final Script script, final Map<String, List<DataEntry>> data) throws Exception {
        if (script == null || data == null) {
            throw new IllegalArgumentException();
        }
        return RFacade.runScriptToGetScalar(script, data);
    }
}
