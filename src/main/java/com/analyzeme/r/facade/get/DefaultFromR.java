...
public class DefaultFromR implements IFromR<NotParsedResult> {
     ...
     */
    public NotParsedResult runScript(final Script script,
                                     int userId, String projectId) throws Exception {
        if (script == null || userId == 0 ||
                projectId == null || projectId.equals("")) {
            throw new IllegalArgumentException();
        }
        return RFacade.runScriptDefault(script, userId, projectId);
    }
    ...
    public NotParsedResult runScript(final Script script, final DataArray data) throws Exception {
        ...
        return RFacade.runScriptDefault(script, data);
    }
}
